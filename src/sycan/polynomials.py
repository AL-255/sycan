...
with the rest of an expression; otherwise a fresh ``cas.Symbol("s")``
is created.
"""
...
def _default_s(s):
    return cas.Symbol("s") if s is None else s


def _poly_from_poles(poles, s) -> cas.Expr:
    ...
    expr = cas.S.One
    for p in poles:
        expr *= (s - cas.sympify(complex(p)))
    coeffs = cas.Poly(cas.expand(expr), s).all_coeffs()  # high-to-low degree
    coeffs = [complex(c).real for c in coeffs]
    n = len(coeffs) - 1
    return sum(cas.Float(c, 6) * s**(n - i) for i, c in enumerate(coeffs))
...
def butterworth(n: int, s: cas.Symbol | None = None) -> tuple[cas.Expr, cas.Expr]:
    s = _default_s(s)
    poles = [
...
def chebyshev1(
    ...
    num = cas.Float(H0 * float(denom.subs(s, 0)), 6)
    return num, denom
...
def bessel(n: int, s: cas.Symbol | None = None) -> tuple[cas.Expr, cas.Expr]:
    s = _default_s(s)
    a = [cas.factorial(2*n - k) / (cas.Integer(2)**(n - k)
                                  * cas.factorial(k) * cas.factorial(n - k))
         for k in range(n + 1)]

...
const coords = document.getElementById('sb-coords') as HTMLElement;
const sbMode = document.getElementById('sb-mode') as HTMLElement;
const sbSel = document.getElementById('sb-sel') as HTMLElement;
const sbZoomPct = document.getElementById('sb-zoom-pct') as HTMLButtonElement;
...
function render(): void {
  ...
  for (const p of state.parts) {
    ...
    for (const term of partTerminals(p)) {
      const k = `${term.pos[0]},${term.pos[1]}`;
      if (!occupied.has(k) && (termCount.get(k) || 0) <= 1) open.add(term.name);
    }
    const g = drawPart(p, {
      hitParent: hitLayer,
...
function updateStatusBar(): void {
  ...
  if (state.selectedIds.size) {
    ...
    let np = 0, nw = 0;
    for (const id of state.selectedIds) { if (partIds.has(id)) np++; else nw++; }
    const frags: string[] = [];
...
// Pinned nets (Shift+click with the Highlight tool). Session-only;
// each pin keeps a distinct hue so several nets can be traced at
// once. Hues picked to read at 0.42 opacity on both themes.
interface PinnedNet { node: string; gridPoints: Set<string>; hue: number }
let pinnedNets: PinnedNet[] = [];
const PIN_HUES = [24, 142, 262, 320, 82, 198];

function pinCurrentNetHighlight(): void {
...
function unpinNet(node: string): void {
  pinnedNets = pinnedNets.filter(pn => pn.node !== node);
  updateNetLegend();
  render();
}

function updateNetLegend(): void {
...
function drawOneNetWash(set: Set<string>, stroke: string | null): void {
  ...
  for (const w of state.wires) {
    ...
    if (w.points.length && w.points.some(([x, y]) => isPt(x, y))) {
      ...
      if (stroke) attrs.style = `stroke: ${stroke}`;
      el('path', attrs, svg);
    }
  }
  for (const p of state.parts) {
    for (const t of partTerminals(p)) {
      if (isPt(t.pos[0], t.pos[1])) {
        ...
        if (stroke) attrs.style = `fill: ${stroke}`;
        el('circle', attrs, svg);
      }
    }
  }
...
function refreshHint(): void {
  ...
  if (hintTimer === null) hint.innerHTML = h;
}
...
wrap.addEventListener('mouseup', (e: MouseEvent) => {
  if (panning) {
    ...
    if (e.button === 2) maybeOpenContextMenu(e);
    return;
  }
...
let rightPress: { x: number; y: number; px: number; py: number } | null = null;

wrap.addEventListener('contextmenu', (e: Event) => e.preventDefault());

function maybeOpenContextMenu(e: MouseEvent): void {
  const press = rightPress;
  rightPress = null;
  if (!press) return;
  const moved = Math.hypot(e.clientX - press.x, e.clientY - press.y);
  if (moved >= 4) return;                   // right-drag pan
  ...
  if (state.moveDraft || state.wireDraft || state.copyAnchorPending
      || state.calcNode.armed || selectGesture || state.boxSelect) return;
...
wrap.addEventListener('click', (e: MouseEvent) => {
  ...
  switch (state.tool) {
    ...
    case 'highlight':
      ...
      if (e.shiftKey && state.netHighlightOverlay) pinCurrentNetHighlight();
      break;
...
function setZoom(newZoom: number, ax?: number, ay?: number): void {
...
wrap.addEventListener('wheel', (e: WheelEvent) => {
  ...
  if (e.ctrlKey || e.metaKey) {
    ...
    const rect = wrap.getBoundingClientRect();
    setZoom(state.zoom * Math.exp(-dy * 0.012),
...
function addPart(type: ElemKind, x: number, y: number, rot: number): void {
  ...
  if (state.tool === type) noteRecentPart(type);
}
...
// ------------------------------------------------------------------
// Command registry — single source for the right-click context menu,
// the Ctrl+K command palette, and the '?' cheat sheet. The document
// keydown handler stays authoritative for key dispatch; commands call
// the same named functions it does.
// ------------------------------------------------------------------

interface CmdCtx {
  world: Point;                 // raw world coords of invocation
  snapped: Point;               // snapPt(world)
  hit: Hit | null;              // pickAt(world); null from the palette
  source: 'menu' | 'palette';
}

type MenuSection = 'part' | 'wire' | 'net' | 'canvas';

interface Command {
  id: string;
  title: string | ((ctx: CmdCtx) => string);
  group: 'Tools' | 'Edit' | 'Selection' | 'View' | 'File' | 'Place';
  shortcut?: string;            // display only; ' / ' splits into <kbd>s
  enabled?: (ctx: CmdCtx) => boolean;
  run: (ctx: CmdCtx) => void;
  menu?: { section: MenuSection; order: number };
  palette?: boolean;            // default true
  cheat?: boolean;              // contributes a cheat-sheet row
  danger?: boolean;
}
...
function expandSelectionToNet(wireId: string): void {
...
function rotateSelection(): void {
  ...
  for (const id of state.selectedIds) {
    const pp = state.parts.find(x => x.id === id);
    if (pp) { pp.rot = (pp.rot + 90) % 360; did = true; }
  }
  if (did) { pushHistory(); render(); }
}
...
function writeRecentList(key: string, list: string[]): void {
  try { localStorage.setItem(key, JSON.stringify(list)); } catch (_) { /* full */ }
}
function noteRecentCommand(id: string): void {
  const list = readRecentList(RECENT_CMD_KEY).filter(x => x !== id);
  list.unshift(id);
  writeRecentList(RECENT_CMD_KEY, list.slice(0, 7));
}
function noteRecentPart(kind: string): void {
  const list = readRecentList(RECENT_PART_KEY).filter(x => x !== kind);
  list.unshift(kind);
  writeRecentList(RECENT_PART_KEY, list.slice(0, 3));
}
function recentParts(): ElemKind[] {
  const list = readRecentList(RECENT_PART_KEY);
  const seed = list.length ? list : ['res', 'vsrc', 'gnd'];
  return seed.filter(k => k in ELEM_TYPES) as ElemKind[];
}

function selectSimilar(partId: string): void {
  const seed = state.parts.find(pp => pp.id === partId);
  if (!seed) return;
  state.selectedIds = new Set(
    state.parts.filter(pp => pp.type === seed.type).map(pp => pp.id));
  state.selectedSegments.clear();
  refreshProps();
  render();
...
function renumberParts(): void {
  if (!state.parts.length) return;
  const ordered = [...state.parts].sort((a, b) =>
  ...
  for (const pp of state.parts) {
    ...
    if (pp.id !== next) changed++;
    // Default values track the id (e.g. R1's value "R1") — keep them
    // in sync when they were never customised.
    if (pp.value === pp.id) pp.value = next;
    pp.id = next;
  }
  for (const pp of state.parts) {
    if (pp.ctrlSrc && idMap.has(pp.ctrlSrc)) pp.ctrlSrc = idMap.get(pp.ctrlSrc);
  }
  state.selectedIds = new Set(
    [...state.selectedIds].map(id => idMap.get(id) ?? id));
...
const clipboardFull = () =>
  ((clipboard.parts?.length ?? 0) + (clipboard.wires?.length ?? 0)) > 0;
...
const COMMANDS: Command[] = [
  ...
  { id: 'edit.undo', title: 'Undo', group: 'Edit', shortcut: 'Ctrl+Z', cheat: true,
    ...
    run: () => (document.getElementById('btn-undo') as HTMLButtonElement).click() },
  { id: 'edit.redo', title: 'Redo', group: 'Edit', shortcut: 'Ctrl+Y', cheat: true,
    enabled: () => historyIdx < editHistory.length - 1,
    run: () => (document.getElementById('btn-redo') as HTMLButtonElement).click() },
  ...
  { id: 'file.importJson', title: 'Import JSON…', group: 'File',
    run: () => (document.getElementById('btn-import-json') as HTMLButtonElement).click() },
  { id: 'file.clearAll', title: 'Clear schematic…', group: 'File', danger: true,
    enabled: () => state.parts.length > 0 || state.wires.length > 0,
    run: () => (document.getElementById('btn-clear') as HTMLButtonElement).click() },
];
// Place commands for every part kind (palette).
for (const kind of Object.keys(ELEM_TYPES) as ElemKind[]) {
  const letter = ({ res: 'R', ind: 'L', cap: 'C', vsrc: 'V', isrc: 'I',
                    diode: 'D', gnd: 'G' } as Record<string, string>)[kind];
  COMMANDS.push({
    id: `place.${kind}`,
...
function makeCtx(world: Point, hit: Hit | null, source: CmdCtx['source']): CmdCtx {
...
function cmdTitle(c: Command, ctx: CmdCtx): string {
  return typeof c.title === 'string' ? c.title : c.title(ctx);
}
function runCommand(c: Command, ctx: CmdCtx): void {
  if (c.enabled && !c.enabled(ctx)) return;
  noteRecentCommand(c.id);
  c.run(ctx);
}

// Render a shortcut label as <kbd> chips; ' / ' separates alternates.
function kbdHtml(label: string): string {
...
const MENU_SECTION_ORDER: MenuSection[] = ['part', 'wire', 'net', 'canvas'];

function sectionVisible(section: MenuSection, hit: Hit | null): boolean {
  switch (section) {
...
function openContextMenu(sx: number, sy: number): void {
  closeContextMenu();
  closeCmdPalette();
  if (shortcutOverlayEl) toggleShortcutOverlay();
  const world = eventToWorld({ clientX: sx, clientY: sy } as MouseEvent);
  const hit = pickAt(world);
  ...
  if (hit && !state.selectedIds.has(hit.id)) {
    ...
    state.selectedSegments.clear();
    if (hit.kind === 'wire') selectWholeWire(hit.id);
    refreshProps();
    render();
  }
  ...
  interface Row { el: HTMLElement; cmd: Command }
  const rows: Row[] = [];
  let lastSection: MenuSection | null = null;

  const addItem = (cmd: Command, title: string) => {
    ...
    if (enabled) rows.push({ el: item, cmd });
    return item;
  };

  for (const section of MENU_SECTION_ORDER) {
    if (!sectionVisible(section, hit)) continue;
    const cmds = COMMANDS
    ...
    lastSection = section;
    for (const cmd of cmds) addItem(cmd, cmdTitle(cmd, ctx));
  }
  ...
  if (!hit) {
    ...
    if (recents.length) {
      ...
      for (const kind of recents) {
        ...
        if (cmd) addItem(cmd, cmdTitle(cmd, ctx));
      }
    }
  }
  ...
  let left = sx, top = sy;
  if (left + r.width > window.innerWidth - 8) left = Math.max(8, sx - r.width);
  if (top + r.height > window.innerHeight - 8) top = Math.max(8, sy - r.height);
  ...
  const setActive = (idx: number) => {
    ...
    active = idx;
    if (active >= 0) rows[active].el.classList.add('active');
  };
  ...
  const onKey = (ev: KeyboardEvent) => {
    if (ev.key === 'Escape') {
      ...
      const n = rows.length;
      if (n) setActive(((active + dir) % n + n) % n);
  ...
  const onDown = (ev: MouseEvent) => {
    if (ctxMenuEl && !ctxMenuEl.contains(ev.target as Node)) closeContextMenu();
  };
...
// ------------------------------------------------------------------
let cmdkEl: HTMLElement | null = null;

function closeCmdPalette(): void {
  if (cmdkEl) { cmdkEl.remove(); cmdkEl = null; }
}
...
function fuzzyMatch(query: string, haystack: string):
  ...
  for (let qi = 0; qi < q.length; qi++) {
    ...
    for (let k = hi; k < h.length; k++) {
      if (h[k] === ch) { found = k; break; }
    }
    if (found === -1) return null;
    score += 1;
    if (found === prev + 1) score += 2;                       // consecutive
...
function toggleCmdPalette(): void {
  if (cmdkEl) { closeCmdPalette(); return; }
  closeContextMenu();
  if (shortcutOverlayEl) toggleShortcutOverlay();
  ...
  const eligible = () => COMMANDS.filter(c =>
    c.palette !== false && (!c.enabled || c.enabled(ctx)));

  interface Entry { cmd: Command; el: HTMLElement }
  let entries: Entry[] = [];
  let active = 0;
  ...
  const refill = () => {
    ...
    active = 0;
    if (entries.length) entries[0].el.classList.add('active');
  };
  ...
  overlay.addEventListener('mousedown', (ev) => {
    if (ev.target === overlay) closeCmdPalette();
  });
...
// aren't commands.
// ------------------------------------------------------------------
const STATIC_SHORTCUT_ROWS: Record<string, Array<[string, string]>> = {
  Selection: [
...
function isConnectionTarget(pt: Point): boolean {
  const [x, y] = pt;
  for (const pp of state.parts) {
    for (const term of partTerminals(pp)) {
      if (term.pos[0] === x && term.pos[1] === y) return true;
    }
  }
  for (const w of state.wires) {
    if (w.bad || w.points.length < 2) continue;
    for (const p2 of w.points) {
      if (p2[0] === x && p2[1] === y) return true;
    }
    for (let i = 0; i < w.points.length - 1; i++) {
      const a = w.points[i], b = w.points[i + 1];
      if (a[0] === b[0] && x === a[0]
      ...
      if (a[1] === b[1] && y === a[1]
          && x > Math.min(a[0], b[0]) && x < Math.max(a[0], b[0])) return true;
    }
  }
  return false;
}
...
function exportSchematicSvg(): string | null {
  ...
  clone.querySelectorAll(
    '.erc-marker, .net-highlight, .net-highlight-term, ' +
    '.calc-node-highlight, .matrix-part-highlight, .preview, ' +
    '.snap-ring, .flash-halo'
  ).forEach(n => n.remove());
...
function setAsyncProgress(track: HTMLElement | null, frac: number | null): void {
  if (!track) return;
  if (frac === null) { track.hidden = true; return; }
  track.hidden = false;
...
function setBtnBusy(btn: HTMLButtonElement, busy: boolean): void {
  btn.disabled = busy;
...
function diffFlash(before: Map<string, string>): void {
  const changed: string[] = [];
  for (const pp of state.parts) {
    const sig = `${pp.x},${pp.y},${pp.rot},${pp.flip ? 1 : 0},${pp.value ?? ''}`;
    if (before.get(pp.id) !== sig) changed.push(pp.id);
  }
  markFlash(changed);
}
function partSigs(): Map<string, string> {
  return new Map(state.parts.map(pp =>
    [pp.id, `${pp.x},${pp.y},${pp.rot},${pp.flip ? 1 : 0},${pp.value ?? ''}`]));
}
...
function markToolbarRowStarts(): void {
  for (const row of document.querySelectorAll<HTMLElement>('.toolbar-row')) {
    let prevTop: number | null = null;
    for (const g of row.querySelectorAll<HTMLElement>(':scope > .group')) {
      g.classList.toggle('row-start',
        prevTop !== null && g.offsetTop !== prevTop);
      prevTop = g.offsetTop;
    }
  }
}
...
const FLASH_MS = 600;
const flashParts = new Map<string, number>();   // part id -> expiry ts
let flashRaf = 0;

function markFlash(ids: Iterable<string>): void {
  const until = Date.now() + FLASH_MS;
  for (const id of ids) {
    if (state.parts.some(pp => pp.id === id)) flashParts.set(id, until);
  }
  if (!flashRaf && flashParts.size) {
    const tick = () => {
      const now = Date.now();
      for (const [id, exp] of flashParts) {
        if (exp <= now) flashParts.delete(id);
      }
      render();
...
function drawFlashHalos(): void {
  if (!flashParts.size) return;
  const now = Date.now();
  for (const [id, exp] of flashParts) {
    const pp = state.parts.find(x => x.id === id);
    if (!pp || exp <= now) continue;
...
let starterCardEl: HTMLElement | null = null;

function loadExampleCircuit(): void {
...
function updateStarterCard(): void {
  const empty = !state.parts.length && !state.wires.length;
  let dismissed = false;
  try { dismissed = localStorage.getItem(WELCOME_KEY) === '1'; } catch (_) { /* ok */ }
  const want = empty && !dismissed;
  if (!want) {
    if (starterCardEl) { starterCardEl.remove(); starterCardEl = null; }
    return;
  }
  if (starterCardEl) return;
  const card = document.createElement('div');
  ...
  card.querySelector('.starter-close')!.addEventListener('click', () => {
    try { localStorage.setItem(WELCOME_KEY, '1'); } catch (_) { /* ok */ }
    updateStarterCard();
  });
...
{
  const more = document.getElementById('btn-more-parts') as HTMLButtonElement;
  const pop = document.getElementById('more-parts-pop') as HTMLElement;
  const onKey = (ev: KeyboardEvent) => {
    if (ev.key === 'Escape') {
      closeMore();
      more.focus();
  ...
  pop.addEventListener('click', (e: MouseEvent) => {
    ...
    if ((e.target as HTMLElement).closest('.tool[data-tool]')) closeMore();
  });
  document.addEventListener('mousedown', (e: MouseEvent) => {
    if (!pop.hidden && !pop.contains(e.target as Node)
        && e.target !== more && !more.contains(e.target as Node)) {
      closeMore();
    }
  }, true);
}
...
document.getElementById('sb-zoom-out')!.addEventListener('click', (e: Event) => {
  setZoom(state.zoom / ZOOM_STEP);
  (e.currentTarget as HTMLElement).blur();
});
sbZoomPct.addEventListener('click', (e: Event) => {
  setZoom(1);
  (e.currentTarget as HTMLElement).blur();
});
...
function fitBounds(xmin: number, ymin: number,
                   xmax: number, ymax: number, maxZoom: number): void {
  const pad = 40;
  const W = wrap.clientWidth, H = wrap.clientHeight;
...
function zoomToSelection(): void {
  if (!state.selectedIds.size) { fitView(); return; }
  let xmin = Infinity, ymin = Infinity, xmax = -Infinity, ymax = -Infinity;
  for (const pp of state.parts) {
    if (!state.selectedIds.has(pp.id)) continue;
    const [x0, y0, x1, y1] = partBBox(pp);
    xmin = Math.min(xmin, x0); ymin = Math.min(ymin, y0);
    xmax = Math.max(xmax, x1); ymax = Math.max(ymax, y1);
  }
  for (const w of state.wires) {
    if (!state.selectedIds.has(w.id)) continue;
    for (const [x, y] of w.points) {
      xmin = Math.min(xmin, x); ymin = Math.min(ymin, y);
      xmax = Math.max(xmax, x); ymax = Math.max(ymax, y);
    }
  }
  if (xmin === Infinity) { fitView(); return; }
...
document.addEventListener('keydown', (e: KeyboardEvent) => {
  ...
  if (k === 'f' && !e.altKey) {
    if (e.shiftKey && state.selectedIds.size) zoomToSelection();
    else fitView();
    e.preventDefault();
    return;
  }
...
function pasteClipboard(at?: Point): void {
  ...
  let anchor: Point = snapPt(at ?? state.cursorWorld);
...
function ensureSycan(
  onStatus: (msg: string, frac: number) => void = () => {},
): Promise<any> {

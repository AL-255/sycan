...
if (!wrapElMaybe) return;   // not the viewer page — nothing to do
...
const wrapEl: HTMLElement = wrapElMaybe;
const svgEl = document.getElementById('viewer-svg') as unknown as SVGSVGElement;
const msgEl = document.getElementById('viewer-msg')!;
const controlsEl = document.getElementById('viewer-controls')!;
const openLink = document.getElementById('v-open') as HTMLAnchorElement;
...
// ---- input decoding ----------------------------------------------
function rawCircuitParam(): string | null {
...
function applyPageOptions(): void {
  const q = new URLSearchParams(location.search);
  const theme = q.get('theme');
  if (theme === 'light' || theme === 'dark') {
    document.documentElement.dataset.theme = theme;
  }
  if (q.get('controls') === '0') controlsEl.hidden = true;
}
...
// workbench.
function vWirePath(points: Point[]): string {
...
function renderViewer(): void {
  ...
  svgEl.setAttribute('viewBox',
    `${-vpan.x / vzoom} ${-vpan.y / vzoom} ${W / vzoom} ${H / vzoom}`);
  ...
  for (const w of vwires) {
    if (w.bad || !w.points || w.points.length < 2) continue;
    for (const pt of w.points) occupied.add(`${pt[0]},${pt[1]}`);
  ...
  for (const p of vparts) {
    if (!ELEM_TYPES[p.type]) continue;   // unknown kind in foreign data
    const open = new Set<string>();
    for (const t of partTerminals(p)) {
      const k = `${t.pos[0]},${t.pos[1]}`;
      if (!occupied.has(k) && (termCount.get(k) || 0) <= 1) open.add(t.name);
    }
...
function drawViewerNetLabels(): void {
  const longestSeg = (w: Wire): number => {
    let best = 0;
    for (let i = 1; i < w.points.length; i++) {
      best = Math.max(best,
        Math.abs(w.points[i][0] - w.points[i - 1][0]) +
        Math.abs(w.points[i][1] - w.points[i - 1][1]));
    }
    return best;
  };
  const picked = new Map<string, Wire>();
  for (const w of vwires) {
    const lab = (w.label || '').trim();
    if (!lab || !w.points || w.points.length < 2) continue;
    const cur = picked.get(lab);
    if (!cur || longestSeg(w) > longestSeg(cur)) picked.set(lab, w);
  }
...
// ---- camera ---------------------------------------------------------
function contentBounds(): [number, number, number, number] | null {
  let xmin = Infinity, ymin = Infinity, xmax = -Infinity, ymax = -Infinity;
  for (const p of vparts) {
    if (!ELEM_TYPES[p.type]) continue;
...
function setViewerZoom(next: number, cx?: number, cy?: number): void {
...
function bindInteractions(): void {
  let panning = false;
  let last = { x: 0, y: 0 };
  wrapEl.addEventListener('mousedown', (e: MouseEvent) => {
  ...
  wrapEl.addEventListener('wheel', (e: WheelEvent) => {
    ...
    setViewerZoom(vzoom * Math.exp(-e.deltaY * 0.0015),
...
async function initViewer(): Promise<void> {
  ...
  if (!raw) {
    showMessage('No circuit data',
      'Append a base64 circuit to the URL: viewer.html#data=… — the '
  ...
  if (!doc) {
    showMessage('Couldn’t read this circuit',
      'The #data= payload isn’t valid base64-encoded SEDRA JSON. '
...
void initViewer().then(() => {
  ...
  (window as unknown as Record<string, unknown>).sedraViewer = {
    ready: true,
    parts: () => vparts,
    wires: () => vwires,
    zoom: () => vzoom,

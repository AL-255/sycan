...
# --- Symbolic results ---------------------------------------------------
V_out = cas.simplify(r.node_voltages[cas.Symbol("V(out)")])
print("Operating point — V_out solved with the saturation drain current only:")
print(rf"$$V_{{out}}(V_{{in}}) = {cas.latex(V_out)}$$")
print()

print("Per-MOSFET saturation predicates (each must be ≥ 0):")
for name, (c1, c2) in r.predicates.items():
    print(rf"$${name}\;\;\text{{threshold}}: {cas.latex(c1)} \;>\; 0$$")
    print(rf"$${name}\;\;\text{{overdrive}}: {cas.latex(c2)} \;\geq\; 0$$")
print()

lo, hi = r.interval
print("Headroom interval (closed form):")
print(rf"$$V_{{in}} \in \left[\;{cas.latex(cas.simplify(lo))}\;,\;{cas.latex(cas.simplify(hi))}\;\right]$$")

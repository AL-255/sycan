...
def test_rc_lowpass_psd_at_dc_and_pole():
    ...
    psd_omega = sp.simplify(total.subs(s, sp.I * omega))
    expected = 4 * k_B * T_kelvin * R / (1 + (omega * R * C) ** 2)
    assert sp.simplify(psd_omega - expected) == 0

    # DC asymptote
    assert sp.simplify(psd_omega.subs(omega, 0) - 4 * k_B * T_kelvin * R) == 0
    # −3 dB point: ω = 1/(R C) ⇒ PSD halved
    assert (
        sp.simplify(psd_omega.subs(omega, 1 / (R * C)) - 2 * k_B * T_kelvin * R)
        == 0
    )
